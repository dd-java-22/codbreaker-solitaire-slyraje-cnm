...
public interface GameSummaryDao {
  ...
  String COMPLETED_RANKING_QUERY = """
      ...
      WHERE solved AND pool_size = :poolSize AND code_length = :codeLength
      ORDER BY guess_count ASC, (last_played - started) ASC
      """;
  ...
  @Update
  int update(GameSummary gameSummary);

  @Delete
  int delete(GameSummary gameSummary);
